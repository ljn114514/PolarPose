...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        h0, w0 = x[0].size()[2:]
        x = torch.cat([x[0], \
            F.interpolate(x[1], size=(h0, w0), mode='bilinear', align_corners=False), \
            F.interpolate(x[2], size=(h0, w0), mode='bilinear', align_corners=False), \
            F.interpolate(x[3], size=(h0, w0), mode='bilinear', align_corners=False)], 1)
...
def get_hrnet(cfg, pretrained=None):
